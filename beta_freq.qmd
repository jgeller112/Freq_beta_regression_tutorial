...
fluency_data <- read_csv("https://raw.githubusercontent.com/jgeller112/beta_regression_tutorial/refs/heads/main/manuscript/data/fluency_data.csv")
...
However, for the zero-inflated part of the model, the `Fluency` predictor is margianlly significant, b = `r round(model_zi$Coefficient[4], 3)` , SE = `r round(beta_model_dis_results$SE[4], 3)` , 95% CIs = \[`r round(beta_model_dis_results$CI_low[4], 3)`,`r round(beta_model_dis_results$CI_high[4], 3)`\], p = `r round(beta_model_dis_results$p[4], 3)`.
...
```{r}
la_xbx <- betareg(Accuracy ~ Fluency  | Fluency,
  data = fluency_data)
```
